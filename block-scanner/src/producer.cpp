...
{  
    ...
    {  
        LOG(ERROR) << "****** No valid brokers specified********" << partition_;
        return PRODUCER_INIT_FAILED;         
    }     
...
{   
  //  char errstr[512]={0};
    ...
    int ret = rd_kafka_produce(topic_, partition_, RD_KAFKA_MSG_F_COPY,
            (void*)buffer, (size_t)buf_len, NULL, 0, NULL);  

    if(ret == -1)  
    {  
        LOG(ERROR) << "****Failed to produce to topic  partition *****\n";
        LOG(ERROR) << topic_ << partition_;

        rd_kafka_poll(handler_, 0);  
        return PUSH_DATA_FAILED;  
    }  

    LOG(INFO) << "***Sent bytes to topic: partition:*****\n";
    LOG(INFO) << buf_len << topic_ << partition_;
    rd_kafka_poll(handler_, 0);  
